        ...
        <div class="card bg-white rounded-lg">
            <form action="{{route('labels.update',$label)}}" method="POST" class="card-body p-4 flex flex-col gap-5" >
                ...
                <label class="form-label">
                    Name
                    <input name="name" type="text" class="input input-sm input-bordered w-full " value="{{ $label->name }}">
                </label>
                <!-- END Name -->
                <!-- START Slug -->
                <label class="form-label">
                   Slug
                    <input name="slug" class="input input-sm input-bordered w-full" value="{{$label->slug}}"
